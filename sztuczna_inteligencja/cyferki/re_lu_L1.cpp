...
// Normalize the input data using L1 normalization
...
int main() {
    ...
    //     auto x = dist(gen);
    //     auto y = dist(gen);
    //     double tag = ((x-0.5) * (y-0.5) < 0 ? 0.0 : 1.0);
    //     trainingInputs.push_back({x,y});
    //     trainingTargets.push_back({tag});
    // }
    // Train the network
    neuralNetwork.train(trainingInputs, trainingTargets, 1000);

    // Test the trained network
    vector<vector<double>> test_inputs;
    vector<vector<double>> test_targets;
